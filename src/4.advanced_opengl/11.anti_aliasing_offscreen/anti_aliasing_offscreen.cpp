...
int main()
{
    ...
    // -------------------------
    Shader shader("11.anti_aliasing.vs.glsl", "11.anti_aliasing.fs.glsl");
    Shader screenShader("11.aa_post.vs.glsl", "11.aa_post.fs.glsl");
    ...
    // 我们使用glTexImage2DMultisample来替代glTexImage2D，它的纹理目标是GL_TEXTURE_2D_MULTISAPLE
    ...
    // 和纹理类似，创建一个多重采样渲染缓冲对象并不难。我们所要做的只是在指定（当前绑定的）渲染缓冲的内存存储时
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 我们不能直接在片段着色器中使用多重采样的纹理。但我们能做的是将多重采样缓冲位块传送到一个没有使用多重采样纹理附件的FBO中
