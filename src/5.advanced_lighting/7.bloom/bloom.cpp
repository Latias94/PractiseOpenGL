...
int main()
{
    ...
    // -------------------------
    Shader shader("7.bloom.vs.glsl", "7.bloom.fs.glsl");
    Shader shaderLight("7.bloom.vs.glsl", "7.light_box.fs.glsl");
    Shader shaderBlur("7.blur.vs.glsl", "7.blur.fs.glsl");
    Shader shaderBloomFinal("7.bloom_final.vs.glsl", "7.bloom_final.fs.glsl");
    ...
    // 我们可以渲染场景两次，每次使用一个不同的不同的着色器渲染到不同的帧缓冲中
    // 但我们可以使用一个叫做MRT（Multiple Render Targets多渲染目标）的小技巧，这样我们就能定义多个像素着色器了;有了它我们还能够在一个单独渲染处理中提取头两个图片
    ...
    // 基本上就是不断地切换帧缓冲和纹理去绘制。这样我们先在场景纹理的第一个缓冲中进行模糊，然后在把第一个帧缓冲的颜色缓冲放进第二个帧缓冲进行模糊
    // 接着，将第二个帧缓冲的颜色缓冲放进第一个，循环往复
