...
int main()
{
    ...
    // -------------------------
    Shader shaderGeometryPass("8.1.g_buffer.vs.glsl", "8.1.g_buffer.fs.glsl");
    Shader shaderLightingPass("8.1.deferred_shading.vs.glsl", "8.1.deferred_shading.fs.glsl");
    Shader shaderLightBox("8.1.deferred_light_box.vs.glsl", "8.1.deferred_light_box.fs.glsl");
    ...
    // 我们并没有储存镜面强度到一个单独的颜色缓冲纹理中，因为我们可以储存它单独的浮点值到其它颜色缓冲纹理的alpha分量中。
    // 我们使用RGB纹理来储存位置和法线的数据，因为每个对象只有三个分量；但是我们将颜色和镜面强度数据合并到一起
    // 存储到一个单独的RGBA纹理里面，这样我们就不需要声明一个额外的颜色缓冲纹理了
