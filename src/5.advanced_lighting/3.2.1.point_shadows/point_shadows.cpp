...
int main()
{
    ...
    // -------------------------
    Shader shader("3.2.1.point_shadows.vs.glsl", "3.2.1.point_shadows.fs.glsl");
    Shader simpleDepthShader("3.2.1.point_shadows_depth.vs.glsl", "3.2.1.point_shadows_depth.fs.glsl", "3.2.1.point_shadows_depth.gs.glsl");    
    ...
    // 这个教程中我们将转而使用另外的一个小技巧来做这件事，几何着色器允许我们使用一次渲染过程来建立深度立方体贴图
    ...
    // 由于我们将使用一个几何着色器，它允许我们把所有面在一个过程渲染
    // 我们可以使用glFramebufferTexture直接把立方体贴图附加成帧缓冲的深度附件
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 我们要为每个方向提供一个不同的视图矩阵。用glm::lookAt创建6个观察方向，每个都按顺序注视着立方体贴图的的一个方向：右、左、上、下、近、远
